# kBar 可执行开发方案（V2，更新于 2026-04-07）
...
1. 以 `kBar` 为锚点，将可见扫描窗口固定在“屏幕中线到 `kBar` 左侧”。
...
1. 不尝试依赖公开 API 把 `kBar` 固定到系统菜单栏最右侧，因为这条路线不可靠。
2. 已增加全局快捷键兜底入口，默认是 `⌃⌥⌘K`，并允许用户在设置页通过键盘录制修改。
3. 当用户直接点击 `kBar` 时，面板仍优先锚定到状态栏图标位置。
4. 当用户通过快捷键唤起时，若 `kBar` 仍可见，则面板与左键点击保持一致，继续锚定真实 `kBar` 图标。
5. 当 `kBar` 已被系统隐藏、拿不到稳定 frame 时，面板会回退到目标屏幕右上角下方的固定位置，而不是再跟随鼠标 `x` 漂移。
...
### 3.5 `kBar` 自激活时的缓存保护（已实现）

1. `kBar` 是 `LSUIElement` 应用；打开设置页或录制快捷键时，它自己会临时成为前台应用。
2. 在这段时间里，系统菜单栏视觉上仍可能保留外部 App 的文本菜单，因此不能把“前台 bundle id = kBar”直接当成有效扫描上下文。
3. 当前会跳过 `kBar` 自己活跃期间的自动刷新，避免把外部 App 的文本菜单误收进虚拟菜单栏缓存。
...
2. 打包脚本位于 [`scripts/package-dmg.sh`](scripts/package-dmg.sh)，输出到 `dist/`，文件名格式为 `kBar-<version>-unsigned.dmg`。
3. `DMG` 内容固定为 `kBar.app + /Applications` 快捷方式，遵循 macOS 常见的“拖到 Applications”安装方式。
...
当前状态：已落地，左键点击 `kBar` 已支持面板开关；面板打开已改为优先显示、再按需补刷新；当前已补齐前台应用切换时的 dirty cache 处理，避免先闪出旧菜单；`kBar` 自己变成前台应用时会跳过污染性刷新，并在回到外部 App 后补刷新；隐藏图标命中回退交互时，会临时抑制自动刷新。
...
当前状态：已实现纯图标展示、外部点击关闭、跨 Space 使用；并已修复“面板打开时再次点击 `kBar` 会被外部点击监听抢先关闭后又重新打开”的事件竞争问题；当前入口已扩展为“菜单栏点击 + 全局快捷键”双通道；状态栏图标已改为优先读取 `Assets.xcassets/StatusIcon.imageset`；应用图标已补齐 `Assets.xcassets/AppIcon.appiconset`，系统权限列表会显示自定义 App Icon；快捷键唤起时会优先贴真实 `kBar`，否则固定回退到目标屏幕右上角；当前台应用切换导致缓存失效时，面板会改为展示“正在刷新...”加载态而不是旧结果。
...
当前状态：主链路可用；隐藏图标发现已接入 `AXExtrasMenuBar` / `runningAppExtras` 探测，并可构建隐藏项镜像模型与专用诊断输出。当前可见扫描窗口固定为“屏幕中线到 `kBar` 左侧”；前台菜单过滤已改为只读取前台 App 的主菜单 `AXMenuBar`，不再把右侧 `AXExtrasMenuBar` 误当成文本菜单区。
...
当前状态：已可用；脚本会在 `dist/` 生成 `kBar-<version>-unsigned.dmg`，供其他 Mac 手动拖入 `Applications` 安装测试。
...
4. 检查辅助功能与录屏权限。
5. 权限齐全后进行扫描并构建镜像模型。
6. 左键点击 `kBar` 以开关方式控制虚拟菜单栏：未显示时优先立即打开，并仅在数据过旧或缓存失效时补一次刷新；已显示时直接收起。

### 5.2 扫描与建模流程

1. 以“屏幕中线到 `kBar` 左侧”作为扫描窗口。
2. AX 通道先取候选：
   - `SystemUIServer`
   - system-wide probe
3. 截图通道切分候选：
...
1. 用户点击 `kBar` 或按下当前配置的全局快捷键打开面板；若面板已显示，则再次触发相同入口可直接收起。
...
6. 面板显示期间，命中 `kBar` 状态栏按钮区域的左键点击不会再被当成“外部点击关闭”，而是交由入口按钮自身的 toggle 逻辑处理。
7. 快捷键唤起时若 `kBar` 已被系统隐藏，面板会固定回退到目标屏幕右上角下方，而不是依赖鼠标位置或失效的状态栏图标位置。
...
1. 扫描区域已固定为“屏幕中线到 `kBar` 左侧”，不再根据前台文本菜单长度二次收窄左边界。
...
3. 某些全屏场景下，`kBar` 本身可能被系统隐藏；当前已由可配置全局快捷键兜底，但快捷键冲突或注册失败时还没有显式 UI 提示。
...
15. 已修复快捷键唤起时面板跟随鼠标位置漂移的问题；当前会优先锚定真实 `kBar` 图标，隐藏时稳定回退到目标屏幕右上角。
16. 已补齐前台应用切换后的缓存失效处理；当前切换前台应用后再次打开面板，不会先闪出旧结果，必要时只会短暂显示“正在刷新...”。
17. 已修复“在设置页修改全局快捷键后，面板被外部 App 文本菜单污染”的问题；当前会跳过 `kBar` 自激活期间的刷新，并在回到外部 App 后强制补一次正确刷新。
18. 已把 `kBar` 菜单栏图标切换为资产目录中的 `StatusIcon`，支持仓库内直接替换图标资源。
...
22. 已将可见扫描窗口固定为“屏幕中线到 `kBar` 左侧”，避免再因前台菜单区推断过窄而漏掉左侧仍然可见的菜单栏图标。
...
25. 已新增无开发者账号场景下的测试打包脚本 `scripts/package-dmg.sh`；当前可生成 `Release` 构建对应的 `dist/kBar-<version>-unsigned.dmg`，用于跨机器测试安装。
...
8. 当 `kBar` 被系统隐藏时的全局快捷键兜底入口：已满足，当前默认快捷键为 `⌃⌥⌘K`，并支持用户在设置页自定义。
