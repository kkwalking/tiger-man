# kBar 可执行开发方案（V2，更新于 2026-04-05）
...
当前状态：设置页已精简为权限状态、手动刷新、全局快捷键录制与“仅展示被隐藏图标”开关；文件诊断落盘默认关闭，需要时通过代码常量开启。设置窗口当前显式关闭了 `NSHostingView` 的自动 sizing，避免 SwiftUI 在窗口布局时反复改尺寸导致崩溃。
