# kBar 可执行开发方案（V2，更新于 2026-04-04）
...
当前状态：已落地，左键点击 `kBar` 已支持面板开关；面板打开已改为优先显示、再按需补刷新；当前已补齐前台应用切换时的 dirty cache 处理，避免先闪出旧菜单；隐藏图标命中回退交互时，会临时抑制自动刷新。
