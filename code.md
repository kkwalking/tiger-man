...
2. 已增加全局快捷键 `⌃⌥⌘K`，作为 `kBar` 被系统隐藏时的备用入口。
3. 当用户直接点击 `kBar` 时，面板仍优先锚定到状态栏图标位置。
4. 当用户通过快捷键唤起时，面板直接使用 fallback anchor，不依赖 `kBar` 图标 frame；当前第一版优先使用鼠标所在屏幕的菜单栏位置。
...
当前状态：已实现纯图标展示、外部点击关闭、跨 Space 使用；并已修复“面板打开时再次点击 `kBar` 会被外部点击监听抢先关闭后又重新打开”的事件竞争问题；当前入口已扩展为“菜单栏点击 + 全局快捷键 `⌃⌥⌘K`”双通道；当前台应用切换导致缓存失效时，面板会改为展示“正在刷新...”加载态而不是旧结果。
...
1. 用户点击 `kBar` 或按下 `⌃⌥⌘K` 打开面板；若面板已显示，则再次触发相同入口可直接收起。
...
7. 快捷键唤起时若 `kBar` 已被系统隐藏，面板直接使用 fallback anchor，不再尝试依赖失效的状态栏图标位置。
...
3. 某些全屏场景下，`kBar` 本身可能被系统隐藏；当前已由全局快捷键兜底，但快捷键组合仍是固定代码常量，尚未支持用户自定义。
...
14. 已新增全局快捷键 `⌃⌥⌘K`，当 `kBar` 被系统隐藏时仍可唤起或收起虚拟菜单栏。
15. 已补齐前台应用切换后的缓存失效处理；当前切换前台应用后再次打开面板，不会先闪出旧结果，必要时只会短暂显示“正在刷新...”。

### 9.2 当前未完成

1. 扩大隐藏图标交互兼容性，减少对回退事件链路的依赖。
2. 补齐更多隐藏图标样本验证，而不只停留在已验证的少量 App。
3. 评估是否需要开放全局快捷键自定义能力，以及注册失败时的显式提示。
...
8. 当 `kBar` 被系统隐藏时的全局快捷键兜底入口：已满足，当前默认快捷键为 `⌃⌥⌘K`。
