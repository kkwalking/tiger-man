...
当前状态：已实现纯图标展示、外部点击关闭、跨 Space 使用；并已修复“面板打开时再次点击 `kBar` 会被外部点击监听抢先关闭后又重新打开”的事件竞争问题；当前入口已扩展为“菜单栏点击 + 全局快捷键”双通道；状态栏图标已改为优先读取 `Assets.xcassets/StatusIcon.imageset`；快捷键唤起时会优先贴真实 `kBar`，否则固定回退到目标屏幕右上角；当前台应用切换导致缓存失效时，面板会改为展示“正在刷新...”加载态而不是旧结果。
