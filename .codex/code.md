# TigerMan 可执行开发方案（V2，更新于 2026-05-07）
...
当前状态：已实现纯图标展示、外部点击关闭、跨 Space 使用；并已修复“面板打开时再次点击 `TigerMan` 会被外部点击监听抢先关闭后又重新打开”的事件竞争问题；当前入口已扩展为“菜单栏点击 + 全局快捷键”双通道，其中左键负责开关虚拟菜单栏，右键弹出包含“偏好设置”和“退出”的菜单，且“退出”项显示标准 `⌘Q` 快捷键提示；状态栏图标已改为优先读取 `Assets.xcassets/StatusIcon.imageset`，当前使用 `cute-tiger-menu-bar` 系列 SVG 资源；应用图标已补齐 `Assets.xcassets/AppIcon.appiconset`，系统权限列表会显示自定义 App Icon；快捷键唤起时会优先贴真实 `TigerMan`，否则固定回退到目标屏幕右上角；当前台应用切换导致缓存失效时，面板会改为展示“正在刷新...”加载态而不是旧结果。
...
7. 右键点击 `TigerMan` 时，状态栏入口会弹出包含“偏好设置”和“退出”的上下文菜单；点击“偏好设置”后打开设置页，“退出”项会显示标准 `⌘Q` 快捷键提示。
