...
- `open TigerMan.xcodeproj`：在 Xcode 中运行和调试。
...
由于当前以手动验证为主，修改后请记录你验证的场景，例如“左键开关面板”“全局快捷键唤起”“设置页录制快捷键”“全屏场景 fallback 定位”。涉及 UI 或扫描逻辑时，优先验证前台应用切换、权限缺失和 `TigerMan` 被隐藏等边界情况。
...
诊断开关在 `TigerMan/Core/Utils/DiagnosticsFileLogger.swift` 的 `DiagnosticsConfiguration`。默认保持关闭；仅在排障时临时改为 `true` 并重新构建。不要提交本地诊断日志文件。
